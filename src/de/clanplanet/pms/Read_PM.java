...
public class Read_PM extends Activity implements TextToSpeech.OnInitListener {
	...
	public void onInit(int status) {
		if(status == TextToSpeech.SUCCESS) {
			...
			if(lang == TextToSpeech.LANG_MISSING_DATA || lang == TextToSpeech.LANG_NOT_SUPPORTED) {
				Toast.makeText(getApplicationContext(), "Die Sprache fÃ¼r Text To Speech ist bei dir nicht verfÃ¼gbar !", Toast.LENGTH_LONG).show();	
			}
		}
		else {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		betreff = betreff.replace("&#228;", "Ã¤");
		betreff = betreff.replace("&#196;", "Ã„");
		betreff = betreff.replace("&#246;", "Ã¶");
		betreff = betreff.replace("&#214;", "Ã–");
		betreff = betreff.replace("&#252;", "Ã¼");
		betreff = betreff.replace("&#220;", "Ãœ");
